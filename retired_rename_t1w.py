...
for d in os.listdir(path = BIDS_dir):
    if "sub" in d:
        print(d) #which subject is being worked on
        orig_nii = "/dwi/"+d+"_rec-NORM_.nii.gz" #orig nii filename 
        new_nii = "/dwi/"+d+"_rec-NORM_T1w.nii.gz" #new nii filename
        orig_json = "/dwi/"+d+"_rec-NORM_.json" #orig json filename
        new_json = "/dwi/"+d+"_rec-NORM_T1w.json" #new json filename
        os.rename(BIDS_dir+d+orig_nii,
                  BIDS_dir+d+new_nii) #rename .nii file
        os.rename(BIDS_dir+d+orig_json,
                  BIDS_dir+d+new_json) #rename .json file
